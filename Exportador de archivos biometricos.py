    ...
    try:
        ...
        def procesar_hora(hora_str):
            try:
                ...
                hora = pd.to_datetime(
                    hora_str, format="%H:%M:%S", errors="coerce"
                ).time()
                if pd.isna(hora):
                    # Si falla, parseamos sin segundos y añadimos aleatorios
                    hora_sin_segundos = pd.to_datetime(
                        hora_str, format="%H:%M", errors="coerce"
                    )
                    if not pd.isna(hora_sin_segundos):
                        segundos_aleatorios = random.randint(0, 30)
                        hora = hora_sin_segundos.replace(
                            second=segundos_aleatorios
                        ).time()
                return hora
            except:
                return None
