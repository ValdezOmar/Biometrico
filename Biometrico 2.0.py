...
def actualizar_ultima_sincronizacion(db, id_equipo):
    try:
        with db.cursor() as cursor:
            cursor.execute(
                """
                INSERT INTO sincronizaciones (id_equipo, ultima_sincronizacion, created_at, updated_at)
    ...
    def obtener_ultima_sincronizacion(self, id_equipo):
        try:
            with conectar_db() as db:
                ...
                with db.cursor() as cursor:
                    cursor.execute(
                        "SELECT ultima_sincronizacion FROM sincronizaciones WHERE id_equipo = %s",
                        (id_equipo,)
                    )
                    result = cursor.fetchone()
...
if __name__ == '__main__':
    try:
        # Crear tabla de sincronizaciones si no existe
        with conectar_db() as db:
            if db:
                try:
                    with db.cursor() as cursor:
                        ...
                        """)
                    logger.info("Tabla de sincronizaciones verificada/creada")
                except Exception as e:
                    logger.error(f"Error creando tabla de sincronizaciones: {str(e)}", exc_info=True)
